...
function ProfileCard({
  ...
  return (
    <div
      ...
      className={cn(
        "group/profile-card flex flex-row items-center gap-x-2 data-[variant=vertical]:gap-y-3",
...
function ProfileCardDetails({
  ...
  return (
    <div
      ...
      className={cn(
        "flex flex-col group-data-[variant=horizontal]/profile-card:gap-3",
    ...
    >
      {children ?? (
        <>
          {name && <ProfileCardName data-slot="details-name">{name}</ProfileCardName>}
          {body && <ProfileCardBody data-slot="details-body">{body}</ProfileCardBody>}
        </>
      )}
    </div>
  );
...
function ProfileCardVerifiedBadge({ className, ...props }: React.ComponentProps<"svg">) {
